...
Original file is located at
    https://colab.research.google.com/drive/1mzlt6jDg07HG8K_vLfRVW9hfhvIobnFF
"""
...
def generate_prompt(
    ...
    ts_features, include_stats, include_corr, graphs, output_format
    ):
    ...
    if source_type == "Kaggle形式":
        ...
        path_var = single_path if source_type == "単一ファイル" else time_series_path
    ...
    prompt.extend([
        ...
        "# Step 2: データ概要の把握",
        "# ==================================",
        f"読み込んだデータ({data_info_target})の概要を把握するため、`.info()`と`.head()`、`.describe(include='all')`を実行してください。",
    ])
    ...
    if problem_type == "時系列予測":
        tasks.append(f"- **時系列データの前処理**: `{time_col}`列をdatetime型に変換し、インデックスに設定してください。")
        if ts_features:
        ...
        tasks.append("- **前処理**: Kaggle形式の場合はtrainとtestを一度結合し、共通の前処理（カテゴリ変数のワンホットエンコーディング、欠損値の平均値補完など）を実装してください。")
...
with st.sidebar:
    ...
    if problem_type == "時系列予測":
        ...
        time_series_path = st.text_input("時系列CSVファイルのパス", "/content/drive/MyDrive/data/my_timeseries.csv")
    else:
        source_type = st.radio("データの形式", ["単一ファイル", "Kaggle形式"], horizontal=True, key="source_type_static")
        if source_type == "Kaggle形式":
            st.write("Google Drive内のファイルパスを入力してください。")
    ...
    include_stats = st.checkbox("基本統計量の確認", value=True)
    ...
    st.subheader("7. 出力形式を選択")
    if problem_type != "時系列予測":
        output_format = st.radio("出力フォーマット", ["一般的な分析・レポート", "Kaggle形式"], horizontal=True, key="output_format_static")
    else:

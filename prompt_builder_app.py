...
def generate_prompt(
    ...
    if problem_type == "時系列予測":
        ...
        if ts_features:
            ...
            for feature in ts_features: feature_tasks += f"  - {feature}\n"
...
use_ensemble, tune_hyperparams, include_corr = False, False, False
graphs = []
save_dir_path = ""
...
with st.sidebar:
    ...
    problem_type = st.radio("分析の種類を選択", ["分類", "回帰", "時系列予測"], horizontal=True, key="problem_type")
    ...
    if problem_type == "時系列予測":
        ...
        ts_features = st.multiselect("作成したい時系列特徴量", ["時間ベースの特徴量", "ラグ特徴量", "移動平均特徴量", "祝日特徴量"], default=["時間ベースの特徴量", "ラグ特徴量"])
    
    st.subheader("5. モデル戦略を選択")
    default_models = ["LightGBM"] if analysis_goal == "要因分析" or problem_type == "時系列予測" else ["LightGBM", "ロジスティック回帰/線形回帰"]
    ...
    if problem_type == "分類":
        graph_options = ["目的変数の分布", "特徴量の重要度 (SHAP)", "混同行列"]
    elif problem_type == "回帰":
        graph_options = ["目的変数の分布", "特徴量の重要度 (SHAP)", "実績値 vs 予測値プロット"]
    else: 
        graph_options = ["時系列グラフ", "時系列分解図", "ACF/PACFプロット", "特徴量の重要度 (SHAP)"]
